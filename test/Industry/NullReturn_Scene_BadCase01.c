...
/*
 * @scene malloc函数调用结果未判空，在sprintf_s中解引用
 */
void TestBad1()
{
...
// RUN: %klee --output-dir=%t.klee-out --execution-mode=error-guided --mock-external-calls --external-calls=all --check-out-of-memory --libc=klee --skip-not-lazy-and-symbolic-pointers --analysis-reproduce=%s.json %t1.bc
