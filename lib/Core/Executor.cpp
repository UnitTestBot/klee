...
cl::list<Executor::TerminateReason> ExitOnErrorType(
    ...
    cl::values(
        ...
        clEnumValN(Executor::UndefinedBehavior, "UndefinedBehavior", "Undefined behavior detected"),
...
void Executor::allocateGlobalObjects(ExecutionState &state) {
  ...
  unsigned int adressSpaceNum = kmodule->targetData->getProgramAddressSpace();
  ...
  llvm::Type *pointerLowerAddr = llvm::PointerType::get(
      llvm::IntegerType::get(m->getContext(), sizeof(*lowerAddr) * 8),
      adressSpaceNum);
  ...
  llvm::Type *pointerUpperAddr = llvm::PointerType::get(
      llvm::IntegerType::get(m->getContext(), sizeof(*upper_addr) * 8), 0);
    ...
    if (ExternalCalls == ExternalCallPolicy::All) { // don't bother checking uniqueness
      *ai = optimizer.optimizeExpr(*ai, true);
      ...
      if (ce->getWidth() == Context::get().getPointerWidth() &&
          state.addressSpace.resolveOne(
              ce, typeSystemManager->getWrappedType(ati->getType()), op)) {
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  for (ResolutionList::iterator i = rl.begin(), ie = rl.end(); i != ie; ++i) {
    ...
    StatePair branches =
        fork(*unbound, mo->getBoundsCheckPointer(base, 1), true);
  ...
  if (unbound) {
    ...
    for (auto &opSkipped : rlSkipped) {
      addressInSkippedObjects =
          OrExpr::create(addressInSkippedObjects,
                         opSkipped.first->getBoundsCheckPointer(address));
    }
...
ObjectPair Executor::lazyInstantiate(ExecutionState &state, KType *targetType,
                                     bool isAlloca, const MemoryObject *mo) {
...
int Executor::resolveLazyInstantiation(ExecutionState &state) {
  ...
  for (auto i : state.symbolics) {
    ...
    case Expr::Concat: {
      ref<ReadExpr> base = ArrayExprHelper::hasOrderedReads(*dyn_cast<ConcatExpr>(lisource));
      auto parent = base->updates.root->binding;
      if (!parent) {
        return -1;
      }
...
void Executor::setInstantiationGraph(ExecutionState &state, TestCase &tc) {
  ...
  for(size_t i = 0; i < state.symbolics.size(); i++) {
    ...
    auto parent = state.pointers[state.symbolics[i].first->lazyInstantiatedSource];
    // Resolve offset (parent.second)
    ref<ConstantExpr> offset;
    bool success = solver->getValue(state.constraints, parent.second, offset, state.queryMetaData);
    if(!success) klee_error("Offset resolution failure (setInstantiationGraph)");
