...
void ConflictCoreInitializer::addConflictInit(const Conflict &conflict,
    ...
    for (auto &block : function->blocks) {
      if (auto call = dyn_cast<KCallBlock>(block.get())) {
        ...
        if (std::find(functions.begin(), functions.end(), called) !=
            functions.end()) {
          addInit(call->getFirstInstruction(), Target(called->entryKBlock));
        }
      }
    }
  }
  ...
  if (target != targetB) {
    ...
    addInit(from, Target(target));
  }
}

void ConflictCoreInitializer::addInit(KInstruction *from, Target to) {
