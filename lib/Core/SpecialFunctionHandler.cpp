...
#define addDNR(name, handler) { name, \
  ...
  add("__ubsan_handle_alignment_assumption", handleAlignmentAssumption, false),
  add("__ubsan_handle_alignment_assumption_abort", handleAlignmentAssumption, false),
  ...
  add("__ubsan_handle_shift_out_of_bounds_abort", handleShiftOutOfBounds, false),
  ...
  add("__ubsan_handle_vla_bound_not_positive", handleVlaBoundNotPositive, false),
  add("__ubsan_handle_vla_bound_not_positive_abort", handleVlaBoundNotPositive, false),
  add("__ubsan_handle_float_cast_overflow", handleFloatCastOverflow, false),
  add("__ubsan_handle_float_cast_overflow_abort", handleFloatCastOverflow, false),
  add("__ubsan_handle_load_invalid_value", handleLoadInvalidValue, false),
  add("__ubsan_handle_load_invalid_value_abort", handleLoadInvalidValue, false),
  add("__ubsan_handle_implicit_conversion", handleImplicitConversion, false),
  add("__ubsan_handle_implicit_conversion_abort", handleImplicitConversion, false),
  ...
  add("__ubsan_handle_nullability_return_v1", handleNullabilityReturnV1, false),
  add("__ubsan_handle_nullability_return_v1_abort", handleNullabilityReturnV1, false),
...
SpecialFunctionHandler::readStringAtAddress(ExecutionState &state, 
  ...
  if (!state.addressSpace.resolveOne(address, executor.typeSystem->getWrappedType(nullptr), op)) {
    executor.terminateStateOnError(
...
void SpecialFunctionHandler::handleMalloc(ExecutionState &state,
  ...
  executor.executeAlloc(state, arguments[0], false, target, executor.typeSystem->getWrappedType(nullptr));
}
...
void SpecialFunctionHandler::handleMemalign(ExecutionState &state,
  ...
  executor.executeAlloc(state, arguments[1], false, target, executor.typeSystem->getWrappedType(nullptr), false, 0,
                        alignment);
}
...
void SpecialFunctionHandler::handleGetErrno(ExecutionState &state,
  ...
  bool resolved = state.addressSpace.resolveOne(
      ...
      executor.typeSystem->getWrappedType(pointer_errno_addr),
      result);
  if (!resolved)
...
void SpecialFunctionHandler::handleCalloc(ExecutionState &state,
  ...
  ref<Expr> size = MulExpr::create(arguments[0],
                                   arguments[1]);
  executor.executeAlloc(state, size, false, target, executor.typeSystem->getWrappedType(nullptr), true);
}
...
void SpecialFunctionHandler::handleRealloc(ExecutionState &state,
  ...
  if (zeroSize.second) { // size != 0
    ...
    if (zeroPointer.first) { // address == 0
      executor.executeAlloc(*zeroPointer.first, size, false, target, executor.typeSystem->getWrappedType(nullptr));
    } 
    if (zeroPointer.second) { // address != 0
      ...
      for (Executor::ExactResolutionList::iterator it = rl.begin(), 
             ie = rl.end(); it != ie; ++it) {
        executor.executeAlloc(*it->second, size, false, target, executor.typeSystem->getWrappedType(nullptr), false, 
                              it->first.second);
      }
    }
  }
...
void SpecialFunctionHandler::handleDefineFixedObject(ExecutionState &state,
  ...
  MemoryObject *mo = executor.memory->allocateFixed(address, 
                                                    size,
                                                    state.prevPC->inst,
                                                    executor.typeSystem->getWrappedType(nullptr)
                                                    );
...
void SpecialFunctionHandler::handleTypeMismatchV1(ExecutionState &state,
                                                  KInstruction *target,
                                                  std::vector<ref<Expr>> &arguments) {
  auto pointerExpr = arguments[1].get();
  auto pointer = executor.toConstant(state, pointerExpr, "handleTypeMismatchV1");
  if (pointer->isZero()) {
    executor.terminateStateOnError(state, "invalid usage of null pointer",
                                   Executor::UndefinedBehavior);
    return;
  } else {
...
void SpecialFunctionHandler::handleAlignmentAssumption(ExecutionState &state,
                                                       KInstruction *target,
                                                       std::vector<ref<Expr>> &arguments) {
...
void SpecialFunctionHandler::handleShiftOutOfBounds(ExecutionState &state,
                                                    KInstruction *target,
                                                    std::vector<ref<Expr>> &arguments) {
...
void SpecialFunctionHandler::handleBuiltinUnreachable(ExecutionState &state,
                                                      KInstruction *target,
                                                      std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state, "execution reached an unreachable program point",
                                 Executor::UndefinedBehavior);
}
...
void SpecialFunctionHandler::handleVlaBoundNotPositive(ExecutionState &state,
                                                       KInstruction *target,
                                                       std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state,
...
void SpecialFunctionHandler::handleFloatCastOverflow(ExecutionState &state,
                                                     KInstruction *target,
                                                     std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state, "floating point value is outside the range of representable values",
                                 Executor::UndefinedBehavior);
}
...
void SpecialFunctionHandler::handleImplicitConversion(ExecutionState &state,
                                                      KInstruction *target,
                                                      std::vector<ref<Expr>> &arguments) {
...
void SpecialFunctionHandler::handleInvalidBuiltin(ExecutionState &state,
                                                  KInstruction *target,
                                                  std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state,"passing zero to either ctz() or clz(), which is not a valid argument",
                                 Executor::UndefinedBehavior);
}

void SpecialFunctionHandler::handleNonnullReturnV1(ExecutionState &state,
                                                   KInstruction *target,
                                                   std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state, "null pointer returned from function declared to never return null",
                                 Executor::UndefinedBehavior);
}

void SpecialFunctionHandler::handleNullabilityReturnV1(ExecutionState &state,
                                                       KInstruction *target,
                                                       std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state, "null pointer returned from function declared to never return null",
                                 Executor::UndefinedBehavior);
...
void SpecialFunctionHandler::handleNonnullArg(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state, "null pointer passed as argument, which is declared to never be null",
                                 Executor::UndefinedBehavior);
}

void SpecialFunctionHandler::handleNullabilityArg(ExecutionState &state,
                                                  KInstruction *target,
                                                  std::vector<ref<Expr>> &arguments) {
  executor.terminateStateOnError(state, "null pointer passed as argument, which is declared to never be null",
                                 Executor::UndefinedBehavior);
...
void SpecialFunctionHandler::handlePointerOverflow(ExecutionState &state,
  ...
  if (base->isZero() && result->isZero()) {
    executor.terminateStateOnError(state, "applying zero offset to null pointer", Executor::UndefinedBehavior);
  } else if (base->isZero() && !result->isZero()) {
    auto message = formattedMessage(
            "applying non-zero offset %llu to null pointer", resultValue);
    executor.terminateStateOnError(state, message, Executor::UndefinedBehavior);
  } else if (!base->isZero() && result->isZero()) {
    auto message = formattedMessage(
