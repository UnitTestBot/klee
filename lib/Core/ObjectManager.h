...
public:
  ...
  std::map<Target, states_ty> reachedStates;
  std::map<Target, pobs_ty> pobs;
  std::map<std::pair<Path, Target>, ProofObligation *> pathedPobs;
  std::map<Target, propagations_ty> propagations;
