  ...
  std::string whatToIncrease)
{
  ...
  klee_warning("%s%% %s", confidence::toString(confidence).c_str(), out.str().c_str());
}

bool confidence::isConfident(ty conf) {
  return conf > Confident;
}

bool confidence::isVeryConfident(ty conf) {
  return conf > VeryConfident;
}

template<typename ... Args>
std::string string_format( const std::string& format, Args ... args )
{ // from https://stackoverflow.com/a/26221725/10547926
