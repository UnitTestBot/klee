...
public:
  ...
  bool seedToFile(unsigned instructions, unsigned isCompleted, const KTest *bo,
                  std::string path);
...
bool KleeHandler::seedToFile(unsigned instructions, unsigned isCompleted,
                             const KTest *bo, std::string path) {
  if (!kTest_toFile(bo, (path + "ktest").c_str())) {
    return false;
  }
    ...
    if (success) {
      if (WriteSeeds) {
        ...
        for (unsigned i = 0; i < ktest->uninitCoeff + 1; ++i) {
          ...
          if (!seedToFile(steppedInstructions, isCompleted, ktest,
